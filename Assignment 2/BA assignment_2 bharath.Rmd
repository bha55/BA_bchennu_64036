---
title: "BA_assignment-2 Bharath chennu"
output: 
      pdf_document: default
      html_docuemnt : default
      date: "10-28-22"
---
...
```{r}
retail<-read.csv("/Users/bharathreddy/Downloads/Online_Retail.csv")
```
...
```{r}
Diff_shopping <- retail%>%select(CustomerID,New_Invoice_Date) %>% group_by(CustomerID)%>% distinct(New_Invoice_Date)%>% mutate(Diff_Days = New_Invoice_Date-lag(New_Invoice_Date))%>% filter(!is.na(Diff_Days))

mean(Diff_shopping$Diff_Days)
```
